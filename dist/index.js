...
(function(){
  var readFile, join, ref$, tail, any, drop, objToPairs, find, fold, map, keys, values, spawn, fsrcConfig, config, say, readStateFile, profileUsernames, profileForUsername, openChromiumWithProfile, ifErr, isGiven, hasValue, dmenu, argsList, PATH, interpolate, args;
  readFile = require('fs').readFile;
  join = require('path').join;
  ref$ = require('prelude-ls'), tail = ref$.tail, any = ref$.any, drop = ref$.drop, objToPairs = ref$.objToPairs, find = ref$.find, fold = ref$.fold, map = ref$.map, keys = ref$.keys, values = ref$.values;
  ...
  openChromiumWithProfile = function(profile){
    return spawn(config.browsers[0]['cmd'], ["--profile-directory=" + profile], {
      detached: true,
      stdio: 'ignore'
    }).unref();
  };
  ...
  interpolate = function(output, key){
    return output.replace("$" + key, process.env[key]);
  };
  PATH = fold(interpolate, PATH)(
  keys(
  process.env));
  args = {
